...
$document.ready(function () {
	...
	{
		const goToIndex = ()=>{
			window.location.href = window.location.href.replace('akaunt.html', 'index.html')
		}
		if(!localStorage.getItem('email') && (!window.location.href.includes('avtoregenshit') && !window.location.href.includes('akaunt'))){
			...
			window.location.href = window.location.href.replace(b, 'avtoregenshit.html')
		}
		const exit = ()=>{
			localStorage.clear()
		}
		...
		let phone = document.querySelector('.phone').value
			...
			fetch("http://back?funk=register&email=" + email + "&pass=" + password + "&phone=" + phone)
			.then(res => res.json())
			.then( async (data)=>{
		...
		let password =document.querySelector('.pass').value
			...
			fetch("http://back?funk=login&email=" + email + "&pass=" + password)
			.then(res => res.json())
			.then( async (data)=>{
				console.log( await data)
				if(data){
